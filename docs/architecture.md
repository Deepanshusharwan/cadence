...
- **Writes:** every mutation in the Android app -- not just starting/stopping a session or marking a day reduced/leave -- is written to the local store immediately and queued in an outbox; the app pushes each outbox to `backend` in a fixed priority order (items/categories, then anchors, then events, then sessions, then day types, then profile, then weekly review) whenever connectivity is available, so anything that might reference an offline-created category is never pushed before that category itself is. Offline-created entities get a client-generated temp id, shown in the UI immediately; once the real create syncs, every other still-queued write referencing that temp id is rewritten to the real one. A permanent rejection (e.g. a 400 for an invalid edit) is dropped from the queue and surfaced as an error rather than retried forever; a transient failure (offline, 5xx) is retried automatically once connectivity returns. Each entity carries `updated_at` and a sync/idempotency identifier so retried pushes don't double-create records -- except client-generated creates, which don't yet send an idempotency key to the backend, so a request that succeeds server-side but whose response is lost to the client can in principle create a duplicate on retry; this is a known gap, not yet fixed.
